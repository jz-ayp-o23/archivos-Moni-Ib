...
def reencriptar(cadena, desplazamiento):
    """
...
def reencriptar_archivo(entrada, desplazamiento):
    """
    ...
    salida = str(archivo.with_name(archivo.stem + "-RECRIPTO" + archivo.suffix))
    with open(archivo, "r", encoding="utf8") as f_in:
        with open(salida, "w", encoding="utf8") as f_out:
            for linea in f_in:
                f_out.write(reencriptar(linea, desplazamiento))